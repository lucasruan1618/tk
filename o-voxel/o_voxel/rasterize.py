...
def rasterize_mesh_attributes(uvs, faces, vertices, texture_size, ssaa=1) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Bake 3D attributes onto a 2D texture map using Kaolin.
    Replaces nvdiffrast mesh rasterization.

    Args:
        uvs: (V, 2) UV coordinates in range [0, 1]
        faces: (F, 3) Face indices
        vertices: (V, 3) 3D vertex positions to interpolate
        texture_size: int (H=W=texture_size)
        ssaa: supersampling factor (rasterize at texture_size*ssaa, caller downsamples)

    Returns:
        pos: (H*ssaa, W*ssaa, 3) interpolated 3D positions
        mask: (H*ssaa, W*ssaa) bool coverage mask
    """
    ...
    # Rasterize with DIB-R at supersampled resolution
    ...
    pos = pos_interpolated[0]  # (H*ssaa, W*ssaa, 3)
    mask = face_idx[0] >= 0  # (H*ssaa, W*ssaa)
