...
from .rasterize import rasterize_mesh_attributes
import cumesh
...
def to_glb(
    ...
    out_vertices, out_faces, out_uvs, out_vmaps = mesh.uv_unwrap(
        ...
        xatlas_pack_charts_kwargs={"padding": 4},
        return_vmaps=True,
        verbose=verbose,
    )
    ...
    # Setup differentiable rasterizer context (supersampled for better coverage)
    internal_size = texture_size * ssaa
    pos, mask = rasterize_mesh_attributes(
        out_uvs, out_faces, out_vertices, texture_size, ssaa=ssaa
    )
    valid_pos = pos[mask]
    ...
    attrs = torch.zeros(internal_size, internal_size, attr_volume.shape[1], device='cuda')
    ...
    # Downsample supersampled attributes and mask to target texture size
    if ssaa > 1:
        attrs = attrs.permute(2, 0, 1).unsqueeze(0)  # (1, C, H*ssaa, W*ssaa)
        attrs = F.interpolate(attrs, size=(texture_size, texture_size),
                              mode='bilinear', align_corners=False, antialias=True)
        attrs = attrs.squeeze(0).permute(1, 2, 0)  # (H, W, C)
        mask_f = mask.float().unsqueeze(0).unsqueeze(0)  # (1, 1, H*ssaa, W*ssaa)
        mask_f = F.interpolate(mask_f, size=(texture_size, texture_size),
                               mode='bilinear', align_corners=False, antialias=True)
        mask = mask_f.squeeze() > 0.25
    ...
    # Fill UV seam gaps via morphological dilation (faster & more consistent than INPAINT_TELEA)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
    mask_u8 = mask.astype(np.uint8)
    all_attrs = np.concatenate([base_color, metallic, roughness, alpha], axis=-1)
    for _ in range(5):
        dilated = cv2.dilate(all_attrs, kernel, iterations=1)
        fill = mask_u8[..., None] == 0
        all_attrs = np.where(fill, dilated, all_attrs)
        mask_u8 = cv2.dilate(mask_u8, kernel, iterations=1)
    base_color = all_attrs[..., :3]
    metallic = all_attrs[..., 3:4]
    roughness = all_attrs[..., 4:5]
    alpha = all_attrs[..., 5:6]
